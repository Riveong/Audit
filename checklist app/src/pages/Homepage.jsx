...
const Homepage = ({ currentPage, setCurrentPage, isLoggedIn, setIsLoggedIn }) => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [checklistsResponse, violationsResponse, sitesResponse] = await Promise.all([
  ...
  const filteredAndSortedChecklists = useMemo(() => {
    let filtered = checklists.filter(checklist => {
      ...
      // Site filter
      const matchesSite = selectedSite === '' || checklist.site === selectedSite
  ...
  // Pagination
  const totalPages = Math.ceil(filteredAndSortedChecklists.length / ITEMS_PER_PAGE)
  const startIndex = (currentPageNum - 1) * ITEMS_PER_PAGE
  const endIndex = startIndex + ITEMS_PER_PAGE
  const currentChecklists = filteredAndSortedChecklists.slice(startIndex, endIndex)
  ...
  const hasActiveFilters = searchTerm || selectedSite || completionFilter !== 'all' || sortBy !== 'newest'
        ...
        <main className="container mx-auto px-4 py-8">
          <div className="flex justify-center items-center min-h-[400px]">
            <div className="text-center">
              ...
              <p className="mt-4 text-gray-600">Loading inspection checklists...</p>
            </div>
          </div>
        </main>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-8">
        {/* Header Section */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Inspection Checklists</h1>
          <p className="text-gray-600">
            Manage and track inspection findings across all sites ({violations.length} standard violations)
          </p>
          {!isLoggedIn && (
            <div className="mt-3 bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              <p className="text-yellow-800 text-sm">
                💡 <strong>Tip:</strong> Login to mark violations found and add inspection notes!
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          <div className="flex flex-wrap items-center justify-between pt-4 border-t border-gray-200">
            <div className="flex flex-wrap items-center space-x-4 text-sm text-gray-600">
              <span>
                Showing <span className="font-medium">{currentChecklists.length}</span> of{' '}
                <span className="font-medium">{filteredAndSortedChecklists.length}</span> checklists
                {checklists.length !== filteredAndSortedChecklists.length && (
                  <span className="text-gray-400"> (filtered from {checklists.length} total)</span>
                )}
              </span>
            </div>
          ...
          <div className="text-center py-12">
            {checklists.length === 0 ? (
              <>
                ...
                <h2 className="text-xl font-semibold text-gray-600 mb-2">No inspection checklists yet</h2>
                <p className="text-gray-500 mb-4">
                  {isLoggedIn ? 'Create your first inspection checklist in the Admin panel.' : 'Login to create inspection checklists.'}
                </p>
                {isLoggedIn && (
                  <button
              ...
              <>
                <div className="text-gray-400 text-6xl mb-4">🔍</div>
                <h2 className="text-xl font-semibold text-gray-600 mb-2">No checklists match your filters</h2>
              ...
              <div className="bg-white rounded-lg shadow-sm p-6">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    Page <span className="font-medium">{currentPageNum}</span> of{' '}
                  ...
                  <div className="text-sm text-gray-600">
                    Showing {startIndex + 1}-{Math.min(endIndex, filteredAndSortedChecklists.length)} of{' '}
